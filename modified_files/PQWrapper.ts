...
export type AttachmentWrapResult = {
  wrapped: Uint8Array;
  mode: 'passthrough' | 'pq';
};
...
export class PQCrypto {
  ...
  /* =======================
     ATTACHMENT WRAP (KIND=0x03)
     ======================= */

  public async wrapAttachmentData(
    recipientServiceId: string,
    data: Uint8Array
  ): Promise<AttachmentWrapResult> {
    await this.initPQ();

    if (!PQ_ENABLED) {
      return { wrapped: data, mode: 'passthrough' };
    }
    ...
    const tSymStart = performance.now();
    const encrypted = this.encrypt(sharedSecret, nonce, data);
    const tSymEnd = performance.now();
    ...
    this.dlog('[TIMING][PQ][ATTACH_SEND]', {
      pqEncapsulateMs,
      pqSymmetricEncryptMs,
      pqTotalWrapMs,
      dataLen: data.length,
  ...
  public async unwrapAttachmentData(
    senderServiceId: string,
    data: Uint8Array
  ): Promise<Uint8Array> {
    await this.initPQ();

    if (!PQ_ENABLED || data.length === 0) return data;

    let offset = 0;
    const kind = data[offset++];

    if (kind !== KIND_ATTACHMENT) return data;

    const version = data[offset++];
    if (version !== PQ_VERSION) return data;

    const kemLen = (data[offset++] << 8) | data[offset++];
    const kemCt = data.slice(offset, offset + kemLen);
    offset += kemLen;
    ...
    this.dlog('[TIMING][PQ][ATTACH_RECV]', {
      pqDecapsulateMs,
      pqSymmetricDecryptMs,
      pqTotalUnwrapMs,
      dataLen: data.length,
